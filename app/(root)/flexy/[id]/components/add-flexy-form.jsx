...
import { createFlexy, deleteFlexy, updateFlexy } from '@/lib/actions';
...
export const AddFlexyForm = ({
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (initialData) {
        // update post 
        await updateFlexy(initialData._id, {
          title: data.title,
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      // delete post 
      await deleteFlexy(initialData._id);
      router.refresh();
      router.push(`/flexy`);
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 w-full max-w-xl mx-auto mt-5">
            ...
            <FormField
              ...
              name="isNotPayed"
              render={({ field }) => (
                <FormItem className="flex flex-col space-y-2 rounded-lg border p-3 shadow-sm w-fit">
                  <FormLabel>Not Payed</FormLabel>
                  <FormControl>
                    <Switch
                      checked={field.value}
